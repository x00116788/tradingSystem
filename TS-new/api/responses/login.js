 ...
 *
 * @description :: Log the requesting customer in using a passport strategy
...
module.exports = function login(inputs) {
    ...
    Customer.attemptLogin({
      ...
      if (!customer) {
        ...
        // send a 200 response letting the customer agent know the login was successful.
        // (also do this if no `invalidRedirect` was provided)
        if (req.wantsJSON || !inputs.invalidRedirect) {
          return res.badRequest('Invalid customername/password combination.');
        }
        // Otherwise if this is an HTML-wanting browser, redirect to /login.
        return res.redirect(inputs.invalidRedirect);
      }
  
      // "Remember" the customer in the session
  ...
  // NOTE:
  //
  // We could have done all (or part) of this in our CustomerController's `login` action.
  ...
  // anyways.
  //
  // For instance, if we need the backend to support multiple customer roles, with different
